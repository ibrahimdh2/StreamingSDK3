...
namespace FMETP
{
    ...
    public class GameViewEncoder : MonoBehaviour
    {
        ...
        private IEnumerator InvokeEventsCheckerCOR()
        {
            ...
            {
                yield return null;
                while (AppendQueueSendByteFMMJPEG.Count > 0)
                { 
                    OnDataByteReadyEvent.Invoke(AppendQueueSendByteFMMJPEG.Dequeue());
                  
                    
                }

                while (AppendQueueSendByteMJPEG.Count > 0)
                {
                    OnRawMJPEGReadyEvent.Invoke(AppendQueueSendByteMJPEG.Dequeue());
